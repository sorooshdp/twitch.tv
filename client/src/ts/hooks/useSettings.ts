...
export const useSettings = () => {
    ...
    const fetchChannelInfo = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await axios.get("https://twitch-tv-0cde.onrender.com/api/settings/channel", {
    ...
    const updateChannelInfo = useCallback(async (updatedInfo: ChannelInfo) => {
        ...
        try {
            const response = await axios.put(
                "https://twitch-tv-0cde.onrender.com/api/settings/channel",
                updatedInfo,
                {
                    withCredentials: true,
                    headers: {
    ...
    const changePassword = useCallback(async (currentPassword: string, newPassword: string) => {
        ...
        try {
            const response = await axios.patch(
                "https://twitch-tv-0cde.onrender.com/api/settings/password",
